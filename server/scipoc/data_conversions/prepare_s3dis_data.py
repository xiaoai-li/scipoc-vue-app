...
def prepare_s3dis_data(DEFAULT_DATA_DIR,DEFAULT_PLY_OUTPUT_DIR):
    parser = argparse.ArgumentParser()
    parser.add_argument('--folder', '-f', help='Path to data folder',default=DEFAULT_DATA_DIR)
    parser.add_argument('--max_point_num', '-m',
                        help='Max point number of each sample', type=int, default=8192)
    parser.add_argument('--block_size', '-b',
                        help='Block size', type=float, default=1.5)
    parser.add_argument('--grid_size', '-g',
                        help='Grid size', type=float, default=0.03)
    parser.add_argument('--save_ply', '-s',
                        help='Convert .pts to .ply', action='store_true',default=DEFAULT_PLY_OUTPUT_DIR)
    ...
    for area_idx in area_indice:
        ...
        for dataset_idx, dataset in enumerate(datasets):
            ...
            if os.path.exists(dataset_marker):
                print(
                    '{}-{}/{} already processed, skipping'.format(datetime.now(), folder, dataset))
                continue
            ...
            for offset_name, offset in offsets:
                ...
                print(
                    '{}-Computing block id of {} points...'.format(datetime.now(), xyzrgb.shape[0]))
                ...
                print(
                    '{}-Collecting points belong to each block...'.format(datetime.now(), xyzrgb.shape[0]))
                ...
                for block_idx in range(idx_last_non_empty_block + 1):
                    ...
                    for block_split_idx in range(block_split_num):
                        ...
                        data[idx_in_batch,
                             0:point_num, ...] = block_xzyrgb[start:end, :]
                        ...
                        if ((idx + 1) % batch_size == 0) or \
                            ...
                            filename_h5 = os.path.join(
                                folder, dataset, '%s_%d.h5' % (offset_name, idx_h5))
                            print(
